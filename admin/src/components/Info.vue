<template>
  <div class="info-component" style=" overflow:auto" :style="{'max-height':getMaxHeight}">
    <el-form ref="form" :label-width="labelWidth || '140px'" :inline="inline">
      <div class="form-div" v-for="(item,index) in infoOption" :key="index" :class="{hasTip:item.width}"
        :style="{width:(item.width || getWidth),float:getFloat}">
        <el-form-item :label="item.label+'：'" style="width:100%;">
          <expand-dom v-if="item.render || item.scoped" :column="item" :data="infoData" :prop="item.prop"
            :render="item.render" :index="index"></expand-dom>
          <span class="txt" v-else-if="item.type == 'time'">{{infoData[item.prop] | getTime}}</span>
          <span class="txt"
            v-else-if="item.type == 'select'">{{item.selectOPtion && item.selectOPtion[infoData[item.prop]]}}</span>
          <span class="txt" v-else-if="item.format"><span v-html="getFormat(item.format)"></span>{{item.append}}</span>
          <a v-else-if="item.href" :target="item.blank ? '_blank' : ''"
            :href="infoData[item.href]">{{infoData[item.prop]}}</a>
          <template v-else>

            <span class="tip txt" :class="item.className || {}" v-html="infoData[item.prop]" v-if="!item.tip"></span>
            <el-tooltip class="item" effect="dark" :content="infoData[item.prop]" placement="top-start" v-else>
              <div class="tip-content txt" :class="item.className || {}">{{infoData[item.prop]}}</div>
            </el-tooltip>
        ...
        <div class="br-b" v-if="hasLine"></div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      loadSelect() {
        ...
        this.curOption.forEach((item, index) => {
